...
local UISaveGame = class("UISaveGame", UILayer)
local saveSlotUtil = require "ui.saveSlotUtil"
function UISaveGame:ctor(backgroundImage)
    UILayer.ctor(self)
    self._background = cc.Sprite:create(backgroundImage):position(video.center):scale("full"):addTo(self)    
    local logo = cc.Sprite:create("save_logo.png"):anchor(0, 1):position(0, video.height - 15):addTo(self)
    ...
    for y = 0, 1 do
        for x = 0, 3 do
            local slot = ccui.Button:create("slot_back.png", "slot_save_2.png"):addTo(self, 4)
            ...
            slot:addClickEventListener(function(sender)
                ...
                novel.saveGame(novel._savePath .. "save_" .. sender._id .. ".sav")
                self:setVisible(false)
                util.captureNode(util.getLayer(), novel._savePath .. "save_" .. sender._id .. ".jpg", 0.25)
...
function UISaveGame:setBackgroundImage(backgroundImage)
